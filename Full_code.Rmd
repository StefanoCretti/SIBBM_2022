...
```{r}
# r: genes, c: samples
counts_raw   <- as.data.frame(assay(recount_data))      
...
```{r}
gene_functions <- rownames_to_column(top_heat_filt, "gene_name") %>% 
                  left_join(info_genes, by="gene_name") %>%
                  select(gene_type)
...
ht <-Heatmap(
  ...
  row_order =  seq(1, 25, 1),
  column_order = seq(1, 159, 1),
  row_names_gp = gpar(fontsize = 12),
  column_names_gp = gpar(fontsize = 6),
  column_split = factor(c(rep("Healthy", 79), rep("Cancer", 80))),
  column_title = "%s",
  row_split = naming_vector,
  row_title = "%s",
...
jpeg(
  ...
  height = 12, width = 16, units='cm', res = 700
  )
draw(ht)
dev.off()
```
...
publish_gostplot(
  plot_gost, 
  highlight_terms = plot_gost$data$term_id[plot_gost$data$p_value < 10e-12] ,
  width = NA,
  height = 10 , 
  filename = "gplot.png")
```
